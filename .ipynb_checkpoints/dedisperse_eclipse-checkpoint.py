...
def dm_2d(
    ...
    Takes input 2D template and disperses + scatters it over the specified range
    of values, performing least-squares fits of each template to the sub-intervals
    of the data. Input template must be phase-aligned and dedispered to same as
    the out-of-eclipse input data sub-intervals.
    
    Function iterates over the DM and scattering ranges in a brute-force manner,
    ensuring that a goodness-of-fit value is returned for every combination.
    
    Function generates, and saves to disk, arrays of best-fit chi-squares,
    template fit amplitudes, and uncertainties on amplitudes for each fit. The
    arrays each have shape (sub-ints, DM_range, tau_range). The elements of the
    chi-square array correspond to the minimised chi-square value for the fit
    to a particular sub-integration of data with the 2D template dispersed
    and scattered with a particular combination of DM and tau. The amplitude
    array corresponds to the amplitude scale factor applied to the template
    that minimised the chi-square (for each combination of sub-integration, DM
    and tau). The amplitude that corresponds to the best-fitting template can be
    considered a measure of the pulsar flux density for a sub-integration.
    
    Note that the amplitude error values do not take into account the
    uncertainty in the DM and tau. Also the error values are not calculated
    correctly when `free_base: False` (I didn't use this so didn't get time to
    fix it) so you should use a free baseline if you plan on using the amplitude
    errors.
